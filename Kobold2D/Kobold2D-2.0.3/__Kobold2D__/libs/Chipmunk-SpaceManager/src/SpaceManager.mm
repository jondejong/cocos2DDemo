...
{
    [self addWindowContainmentWithFriction:friction 
...
-(void) addWindowContainmentWithFriction:(cpFloat)friction elasticity:(cpFloat)elasticity rect:(CGRect)rect inset:(cpVect)inset radius:(cpFloat)radius
{	
...
{		
	...
	if (!_constantDt)
	{	
		float dt = delta/_steps;
		for(int i=0; i<_steps; i++)
			cpSpaceStep(_space, dt);
	}
	else 
	{
		float dt = _constantDt/_steps;
