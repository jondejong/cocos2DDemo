...
-(BOOL) pixelMaskIntersectsPixelMaskSprite:(KKPixelMaskSprite*)other
{
	...
	for (NSUInteger y = intersectSelf.origin.y; y < maxY; y++)
	{
		...
		{
			NSUInteger index = y * pixelMaskWidth + x;
			NSAssert2(index < pixelMaskSize, @"pixelMask index out of bounds for x,y: %u, %u", (unsigned int)x, (unsigned int)y);
...
#endif
			{
				...
				NSAssert2(otherIndex < other.pixelMaskSize,
						  @"other pixelMask index out of bounds for x,y: %u, %u", (unsigned int)otherX, (unsigned int)otherY);
