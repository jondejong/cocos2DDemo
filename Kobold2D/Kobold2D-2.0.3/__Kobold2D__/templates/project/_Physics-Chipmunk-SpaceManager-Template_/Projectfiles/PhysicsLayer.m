...
-(id) init
{
	if ((self = [super init]))
	{
		...
		{
			CCLabelTTF* warning = [CCLabelTTF labelWithString:@"NOTE: Chipmunk SpaceManager is (currently) not compatible with Cocos2D 2.0. You will experience glitches like non-rotating sprites and non-functional debug drawing."
												   dimensions:CGSizeMake(320, 240)
												   hAlignment:kCCTextAlignmentLeft
												lineBreakMode:kCCLineBreakModeWordWrap
													 fontName:@"Arial" 
													 fontSize:12];
			[self addChild:warning];
			[warning setColor:ccc3(255, 255, 0)];
			warning.opacity = 180;
			warning.position = CGPointMake(screenSize.width / 2, screenSize.height / 6);
		}
	}
